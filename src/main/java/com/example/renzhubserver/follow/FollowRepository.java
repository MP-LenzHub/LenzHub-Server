...
public interface FollowRepository extends JpaRepository<Follow, Follow.PK> {
    @Query(value = "select new com.example.renzhubserver.user.model.response.UserBasicInfoResDto(u.name, u.userId) from Follow f INNER JOIN User u ON f.toUser = u.id where f.fromUser = :userId")
    List<UserBasicInfoResDto> findAllByFromUser(@Param("userId") Long userId);  // 나를 팔로우하는 관계를 가져옴
    @Query(value = "select new com.example.renzhubserver.user.model.response.UserBasicInfoResDto(u.name, u.userId) from Follow f INNER JOIN User u ON f.fromUser = u.id where f.toUser = :userId")
